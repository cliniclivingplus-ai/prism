...
function buildDayProgression(actions: string[]): string[][] {
  return DAY_PROGRESS_MARKERS.map((marker) => actions.map((a) => `${marker}: ${firstSentence(a)}`))
}
function hasValidDays(week: Record<string, unknown>, actionCount: number): boolean {
...
Return only 6 lines, 2 per period, in the order Morning, Morning, Afternoon, Afternoon, Evening, Evening. No intro, no outro, no bullet characters.` }
    ...
    // ── STEP 3B: Meal guidelines ─────────────────────────────
    // A dedicated generation, not derived from the "Diet protocol" clinical
    // note — that section is general dietary intervention text also used
    // for recipe-bank keyword matching (lib/pdf/matchRecipes.ts), so
    // overloading it with meal-timing content would have degraded both.
    // This mirrors Step 3's explicit-label approach: each bullet is
    // pre-labeled Breakfast/Lunch/Dinner so groupBulletsByLabel (used by
    // every Week-family template) sorts it deterministically instead of
    // guessing by keyword or round-robin.
...
Return only 6 lines, 2 per meal, in the order Breakfast, Breakfast, Lunch, Lunch, Dinner, Dinner. No intro, no outro, no bullet characters.` }
    ...
    // ── STEP 3C: Daily schedule ───────────────────────────────
    // The full hour-by-hour day, distinct from the Morning/Afternoon/Evening
    // highlights in Step 3 above — those are a handful of key habit
    // reminders, this is the complete timed itinerary rendered as the
    // "flow map" timeline (see parseScheduleLines / WeekTemplate's
    // "Daily Schedule" section). Same explicit-format approach as Steps 3/
    // 3B: one line per entry, "<time> — <activity>", parsed deterministically
