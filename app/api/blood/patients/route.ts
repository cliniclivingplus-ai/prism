...
// Creates a patient account keyed by a real, clinic-assigned Clinicea ID —
...
// Clinicea ID, which is what report uploads now key off of instead of a
// typed/matched name.
export async function POST(req: NextRequest) {
  try {
    ...
    if (!clinicId) return NextResponse.json({ error: 'Clinicea ID is required' }, { status: 400 })
    ...
    if (error) {
      if (error.code === '23505') {
        return NextResponse.json({ error: `Clinicea ID "${clinicId}" is already in use by another patient.` }, { status: 409 })
      }
